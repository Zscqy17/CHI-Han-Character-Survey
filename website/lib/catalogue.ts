...
export interface Media { id:string; paperId:string; kind:'image'|'gif'|'video'; file?:string; poster?:string; sourceUrl:string; videoUrl?:string; embedUrl?:string; page?:number; figure?:string; caption:Localized; license?:string; licenseUrl?:string; adaptation?:string; timestamps?:number[]; status?:string; sourceKind?:'figure'|'table'|'page_preview'|'figure_sequence'; sourceType?:'conference_explanation'|'paper_figure_slideshow'; frames?:{file:string;figure:string;page:number;rect:number[];sha256:string}[]; rights?:string; captionOriginal?:string; sourcePdfSha256?:string; rect?:number[]; galleryPrimary?:boolean; }
...
export function mediaRank(m?:Media){return !m?4:m.kind==='gif'&&m.file?(m.sourceType==='paper_figure_slideshow'?0.5:0):availableVideo(m)?1:m.kind==='image'&&m.file?2:3;}
...
export function filterPapers(papers:Paper[], f:Filters) {
 ...
 return papers.filter(p=>(!f.view||f.view==='all'||p.views.includes(f.view))
  ...
  &&(!f.scenario||f.scenario==='all'||p.modalities.includes(f.scenario))
