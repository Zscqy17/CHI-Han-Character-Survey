...
import {filterPapers,interactionTopics,partitionGallery,previewMedia,previewImage,orderedMedia,availableVideo,canPlayVideo,assetHref,homeHref,paperHref,viewHref,validLocale,type Paper,type Media,type Locale,type Localized,type Filters} from '@/lib/catalogue';
...
export default function Atlas({initialPaperId,initialView='all',offline=false,includedPdfs=false}:{initialPaperId?:string;initialView?:string;offline?:boolean;includedPdfs?:boolean}) {
 ...
 {view!=='interaction'&&<div className="filter-group"><h3>{t('algorithmGenerations')}</h3><div className="filter-options" role="group" aria-label={t('algorithmGenerations')}>{[['all',t('allApproaches')],['rule',t('generationRule')],['stat_lm',t('generationStat')],['neural',t('generationNeural')],['llm',t('generationLlm')],['hybrid',tag('hybrid')],['na',t('otherApproaches')]].map(([v,l])=><Button key={v} variant="ghost" aria-pressed={approach===v} className="filter-option" onClick={()=>setApproach(v)}><span>{l}</span><span>{count({approach:v})}</span></Button>)}</div><p className="filter-note">{t('generationNote')}</p></div>}
 ...
 {view!=='algorithms'&&<div className="filter-group"><h3>{t('scenario')}</h3><div className="filter-options" role="group" aria-label={t('scenario')}>{[['all',t('allScenarios')],...interactionTopics.map(({code,labelKey})=>[code,t(labelKey as Label)])].map(([v,l])=><Button key={v} variant="ghost" className="filter-option" aria-pressed={scenario===v} onClick={()=>setScenario(v)}><span>{l}</span><span>{count({scenario:v})}</span></Button>)}</div><p className="filter-note">{t('modalityNote')}</p></div>}
