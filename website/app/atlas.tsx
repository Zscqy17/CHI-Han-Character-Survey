'use client';
import {classificationDimensions,classificationNote,type ClassificationKey} from '@/lib/classification';
...
import {filterPapers,paperClassification,partitionGallery,previewMedia,previewImage,orderedMedia,availableVideo,canPlayVideo,assetHref,homeHref,paperHref,viewHref,validLocale,type Paper,type Media,type Locale,type Localized,type Filters} from '@/lib/catalogue';
...
function GifPreview({item,lang,asset,href,loading='lazy'}:{item:Media;lang:Locale;asset:(path:string)=>string;href:string;loading?:'eager'|'lazy'}){
 ...
 useEffect(()=>{if(!('IntersectionObserver' in window)){setVisible(true);return}const observer=new IntersectionObserver(entries=>setVisible(entries.some(e=>e.isIntersecting)),{rootMargin:'200px'});if(container.current)observer.observe(container.current);return()=>observer.disconnect()},[]);
 return <div className="gif-preview" ref={container}><a href={href}><img loading={loading} src={asset((!playing||!visible)&&item.poster?item.poster:item.file!)} alt={item.caption[lang]}/></a>{item.poster&&<Button variant="outline" className="motion-toggle" aria-pressed={playing} onClick={()=>setPlaying(!playing)}>{playing?<Pause size={14}/>:<Play size={14}/>}<span>{labels[playing?'pause':'playGif'][lang]}</span></Button>}</div>;
}
...
function MediaView({item,paper,lang,asset}:{item:Media;paper:Paper;lang:Locale;asset:(path:string)=>string}){
 ...
 {item.frames&&<details><summary>{t('originalFigures')} ({item.frames.length})</summary><ol>{item.frames.map((frame,i)=><li key={i}><a href={asset(frame.file)} target="_blank" rel="noreferrer">{frame.figure&&<>{t(frame.kind==='table'?'figureTable':'figure')} {frame.figure} · </>}{t('page')} {frame.page} ↗</a></li>)}</ol></details>}
...
export default function Atlas({initialPaperId,initialView='all',offline=false,includedPdfs=false}:{initialPaperId?:string;initialView?:string;offline?:boolean;includedPdfs?:boolean}) {
 const [lang,setLang]=useState<Locale>('en');
 const [view,setView]=useState(initialView);
 const [approach,setApproach]=useState('all'),[modality,setModality]=useState('all'),[era,setEra]=useState('all'),[scenario,setScenario]=useState('all'),[evaluation,setEvaluation]=useState('all'),[population,setPopulation]=useState('all');
 ...
 const currentFilters:Filters={tier,script,query,media,view,approach,modality,era,scenario,evaluation,population};
 const filtered=useMemo(()=>filterPapers(papers,{tier,script,query,media,view,approach,modality,era,scenario,evaluation,population}),[tier,script,query,media,view,approach,modality,era,scenario,evaluation,population]);
 const count=(overrides:Partial<Filters>)=>filterPapers(papers,{...currentFilters,...overrides}).length;
 const setClassification=(key:ClassificationKey,value:string)=>({script:setScript,modality:setModality,approach:setApproach,evaluation:setEvaluation,population:setPopulation})[key](value);
 const reset=()=>{setTier('all');setScript('all');setQuery('');setMedia('all');setApproach('all');setModality('all');setEra('all');setScenario('all');setEvaluation('all');setPopulation('all')};
 ...
 {classificationDimensions.map(d=>{const r=paperClassification(paper);const values=d.key==='script'?paper.scriptCodes:d.key==='modality'?paper.modalities:d.key==='approach'?paper.tech:d.key==='evaluation'?r?.evaluation:r?.userGroups;return values?.length?<div className="aside-block" key={d.key}><h2>{d.label[lang]}</h2><div className="tag-list">{values.map(v=><span key={v}>{d.options.find(o=>o.code===v)?.label[lang]||tag(v)}</span>)}</div></div>:null})}
 ...
 {classificationDimensions.map(d=><div className="filter-group" key={d.key}><h3>{d.label[lang]}</h3><div className="filter-options" role="group" aria-label={d.label[lang]}>{[{code:'all',label:labels.all},...d.options].map(o=><Button key={o.code} variant="ghost" className="filter-option" aria-pressed={currentFilters[d.key]===o.code} onClick={()=>setClassification(d.key,o.code)}><span>{o.label[lang]}</span><span>{count({[d.key]:o.code})}</span></Button>)}</div></div>)}
 <p className="filter-note">{classificationNote[lang]}</p>
 ...
 <section className="bibliography-section"><div className="download-block"><h3>{t('downloads')}</h3><a href={asset('data/references.bib')} download><Download size={14}/>BibTeX<ArrowUpRight size={14}/></a></div></section>
 </>}
 </main><footer><a className="wordmark" href={home}>CJK Input Atlas</a><p>{t(offline?'offlineFooter':'footer')}</p></footer>
 </>;
}
